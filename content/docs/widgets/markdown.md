...
* `sanitize_preview`: accepts a boolean value, `true` by default. Sanitizes the markdown preview to prevent XSS attacks and might alter the preview content. *This default changed from `false` to `true` in decap-cms@3.13.0 as a security hardening measure.*
